    ...
    fetch('https://cdnjs.cloudflare.com/ajax/libs/stockfish.js/10.0.0/stockfish.js').then(r => r.blob()).then(b => {
        ...
        stockfish.onmessage = (e) => {
            ...
            if (l.includes('score cp')) {
                ...
                // Only update main bar if master coach is not purely active or we want constant updates
                if (!analysisActive) $('#eval-fill-master').css('height', h + '%');

                // Coach Logic (only if playing against AI and hints/coach active)
                ...
                if (pv && pv[1]) {
                    ...
                    if (currentMode === 'ai' || isJ || analysisActive || hintsActive || currentMode === 'study') {
                        ...
                        // Ev is from White perspective.
                        var diff = 0;
                        if (window.lastEval !== undefined) {
                            // If White just moved (now Black turn), game.turn() is 'b'.
                            // Ev (curr) is white adv (e.g. +1). LastEval was +2.
                            // Diff = 2 - 1 = +1 (I lost 1.0 adv, bad).
                            // If Black just moved (now White), turn 'w'.
                            // ev is +2. LastEval was -3.
                            // Diff (relative to black?) -> 
                            // Let's stick to absolute magnitude of change for warnings.

                            // We want separate "Good/Bad" judgement.
                            // If turn is 'b' (White just moved):
                            // target was +Inf. Current is +1. I lost adv.
                            // If turn is 'w' (Black just moved):
                            // target was -Inf. Current is -1. I lost adv.

                            // Generic "Swing against me" calculation:
                            const turn = game.turn();
                            const prev = window.lastEval;
                            const curr = ev;

                            // If I am White (turn is now Black):
                            if (turn === 'b') { diff = prev - curr; }
                            // If I am Black (turn is now White):
                            else { diff = curr - prev; }
                        }

                        // TRAP DETECTION: If huge swing (> 1.5)
                        let trapMsg = "";
                        if (diff > 1.5) { // 150cp lost
                            // Note: This logic is tricky because engine analyzes AFTER move. 
                            // "Cuidado" warning is usually predictive.
                            // But here we are judging the move JUST MADE.
                            if (!hintsActive) {
                        ...
                        // DETAILED TACTICAL & STRATEGIC ANALYSIS
                        let explanation = '';
                        let tacticalInfo = '';
                        ...
                        // Explanation based on EVAL swing (diff)
                        if (diff > 2.0) {
                        ...
                        $('#coach-txt').html(`
                                    ...
                                    ${precisionMsg}
                                    ${openingMsg}
                                    <div style="font-size:0.75rem; line-height:1.4;">${explanation}</div>
                                    <div style="font-size:0.7rem; color:#3b82f6; margin-top:5px;">${tacticalInfo}</div>
                                    ${hintsActive ? `<div style="margin-top:8px; padding-top:5px; border-top:1px solid rgba(255,255,255,0.1); font-size:0.75rem;">💡 Sugerencia: <b style="color:var(--accent)">${pv[1]}</b></div>` : ''}
                                `);
                        isJ = false;
                        isJ = false;
                    }
            ...
            if (l.startsWith('bestmove') && currentMode === 'ai' && game.turn() !== myColor) {
                ...
                // --- LOW ELO BLUNDER SIMULATION ---
                const diffLvl = parseInt($('#diff-sel').val());
                /* 
                   Level 1 (ELO 500) -> 50% chance of random move
                   Level 3 (ELO 800) -> 35% chance of random move
                   Level 5 (ELO 1100) -> 20% chance of random move
                   Level 8+ -> Play best move (Stockfish depth limited)
                */
                let blunderChance = 0;
                ...
                if (Math.random() < blunderChance) {
                    ...
                    if (legalMoves.length > 0) {
                        ...
                        console.log("🤪 Blunder! AI playing random move:", bestMove);
                    }
                }
                // ----------------------------------
                ...
                board.position(game.fen());
                updateUI(true); // "true" triggers sound, history, and player turn analysis
                checkGameOver();
            }
        };
    });
...
$('#btn-start-ai').click(() => {
    ...
    if (myColor === 'b') {
        // AI Move First
        setTimeout(() => {
            const m = game.move('e4'); // Simple opening or random
            board.position(game.fen());
            updateUI(true);
        }, 500);
    }
...
$('#btn-suggest-move').off('click').on('click', function () { toggleHints(this); });
$('#btn-ai-hint').off('click').on('click', function () { toggleHints(this); });
...
function selectMode(selectedMode) {
    ...
    // For 'friend' mode, we might want to use the 'local' UI and just inhibit socket
...
>>>>>>> 38bd149 (Refactor mobile welcome screen, add Study sub-menu, Study HUD with best move toggle, and local multiplayer mode)
